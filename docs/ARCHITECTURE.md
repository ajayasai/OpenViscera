...
The fingerprint covers case metadata, specimen and seal state, requests, transfers, reports, attachments and substantive notes. It also covers correction, report-withdrawal and opinion-withdrawal control histories. It excludes access-policy changes, ordinary opinion actions and administrative follow-ups. A new evidence attachment or substantive note conservatively makes a previous draft/approval stale, even before a new report has been registered. Schema-3 administrative certificates are excluded from that clinical fingerprint but remain signed and exported; retention and hold histories use a separate disposal-approval fingerprint. Review and approval are not the same act. A later report never inherits the review or approval of the version it supersedes.
...
New stores use database schema 3. The explicit migration from schema 1 or 2 preserves original case rows, signatures and keys; schema-1 upgrades also add access-audit storage. Schema-1 and schema-2 events use their frozen historical reducers; new events use schema-3 rules. Version downgrade inside an event stream is rejected. A successful migration verifies unchanged case heads. Further format changes still require explicit versioned migrations and historical-bundle tests; do not modify the frozen reducer or reinterpret signed old events.

## Schema-3 lifecycle extension

The current reducer is `domain.py`; `domain_v1.py` and `domain_v2.py` are frozen historical interpreters. Schema-3 adds retention, holds/releases, disposal and examiner reassignment under `controls`, using the same signed events, transactional projections and optimistic versions. Administrative certificate attachment bytes remain signed/exported but are excluded from the clinical fingerprint; disposal authorization uses a separate conservative fingerprint over evidence and relevant control histories. See [v0.3 boundaries](V03-UPGRADE.md).
