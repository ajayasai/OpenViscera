...
Administrative changes and successful authentication are recorded in a separate signed administrative chain. Clinical writes are in each case ledger. **Not every read or failed login is an append-only access-audit event.** Login failures are retained for throttling, not represented as a complete forensic access log.
...
There is no SSO, MFA, password-reset/rotation interface, fine-grained restricted-case access within a department, independent key custody, formal key-rotation migration, complete read-access audit, antivirus/CDR service, export approval workflow, legal hold/disposal workflow or high-availability design. Department staff can read department cases. Case exports contain confidential information; human recipients remain responsible for secure handling.
